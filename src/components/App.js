...
const states = [{
	...
	city: [{
		name: "Indore",
		description: "Indore is a city in west-central India. It’s known forr the 7-story Rajwada Palace and the Lal Baag Palace, which date back to Indore’s 19th-century Holkar dynasty.",
		landmarks: [{
			name: "Mhow",
...
function App() {
	...
	const handleLandmarkChange = (event) => {
		...
		console.log("landmark selected", event.target.value)
	...
	return (
		<div id="main">
			...
			<select name="City" id="city" onChange={handleCityChange}>
				{states[stateSelected]?.city.map((cityObj, index) => {
					return (<option value={index} key={uniqid()}>{cityObj.name}</option>)
				})}
			</select>
			...
			{states[stateSelected]?.city[citySelected]?.landmarks.map((landmark, index) => {
									return <option value={index} key={uniqid()} >{landmark.name}</option>
								})
							}
			</select>
			...
			<div>
				...
				<div id="state-description">State Description: {states[stateSelected]?.description} </div>
			</div> <hr></hr>
			<div>
				{/* {console.log(states[stateSelected])} */}
				<div id="city-name">City Name: {states[stateSelected]?.city[citySelected]?.name}</div>
				<div id="city-title">City Title: {states[stateSelected]?.city[citySelected]?.name}</div>
				<div id="city-description">City Description: {states[stateSelected]?.city[citySelected]?.description}</div>
			</div> <hr></hr>
			<div>
				<div id="landmark-name">Landmark Name: {states[stateSelected]?.city[citySelected]?.landmarks[landmarkSelected]?.name} </div>
				<div id="landmark-description">Landmark Title: {states[stateSelected]?.city[citySelected]?.landmarks[landmarkSelected]?.name} </div>
				<div id="landmark-title">Landmark Description: {states[stateSelected]?.city[citySelected]?.landmarks[landmarkSelected]?.description} </div>
			</div> <hr></hr>
		</div>
	);
}
